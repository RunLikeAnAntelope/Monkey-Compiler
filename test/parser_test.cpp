...
static void testLiteralExpression(Ast::IExpression &exp,
  ...
  if (std::holds_alternative<long int>(expectedValue)) {
    ...
    ASSERT_TRUE(false) << "type of exp not handled in testLiteralExpression";
  }
}
...
TEST(Parser, LetStatements) {
  ...
  for (const LetStmtTest &test : tests) {
    ...
    std::unique_ptr<Ast::IStatement> stmt = std::move(program.m_statements[0]);
...
TEST(Parser, ReturnStatements) {
  ...
  for (const auto &tst : tests) {
    ...
    ASSERT_EQ(program.m_statements.size(), 1) << "program.Statements does not"
...
TEST(Parser, BooleanLiteralExpression) {
  ...
  std::vector<booleanTest> tests = {{.input = "true;", .expectedBoolean = true},
                                    {"false;", false}};
...
TEST(Parser, FunctionLiteralParsing) {
  ...
  ASSERT_TRUE(exp != nullptr) << "stmt->m_expression is not an FunctionLiteral";
...
TEST(Parser, CallExpressionParsing) {
  ...
  ASSERT_TRUE(exp != nullptr) << "ExpressionStatement is not a CallExpression";
...
TEST(Parser, TestStringLiteral) {
  ...
  ASSERT_EQ(stmt->m_expression->Type(), Ast::Type::STRING_LITERAL)
    << "exp not a StringLiteral";
